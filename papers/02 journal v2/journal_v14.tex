...
\end{equation}
where $\eta_i$ is the local power unbalance of prosumer $i$, i.e.,
\begin{equation}
\label{eq:etai}
\eta_i := p_i^{\mathrm d}-p_{i}^{\mathrm{di}}-\edit{p_{i}^{\mathrm{ds}}+p_{i}^{\mathrm{ch}}}.
\end{equation}
Equation \eqref{eq:p_bal_p} represents the local power balance of bus $y$, similarly to \eqref{eq:l_pow_b} although now it relates power generation, consumption and line powers. Moreover, it must hold that
\begin{subequations}
\small
\label{eq:pf_pq}
\begin{align}
...
\begin{remark}
Our definitions of $J_i$, $\mc U_i$ and $\mc C$ satisfy Assumption \ref{as:gen}. Moreover, these definitions can be expanded by incorporating additional cost terms, for example, related to the degradation of storage units and constraints (e.g. ramping constraints of dispatchable generation units), as long as Assumption \ref{as:gen} remains satisfied.  \marginnote{\note{R3-2}} \edit{Additionally, instead of considering linear power flow equations in \eqref{eq:pf_all}, a nonlinear convex relaxation, such as a second order cone or semi-definite programming as discussed in \cite[Sect. II-A]{molzahn2017} can be considered since it still satisfies Assumption 1. In this case, only the definition of $\mc U_{N+1}$ differs from the current formulation.}\eod
\end{remark}

\smallskip
From a game-theoretic perspective, the inter-dependent optimization problems in \eqref{eq:locOpt} constitute a \textit{generalized game} \cite{facchinei2010generalized} and a set of decisions $\{u_1^*,\ldots,u_{N+1}^*\}$ that simultaneously satisfy \eqref{eq:locOpt}, for all $i \in \mc N^+$, corresponds to a GNE \cite[\S~1]{facchinei2010generalized}.
%
In other words, a set of strategies $\{u_1^*,\ldots,u_{N+1}^*\}$ is a GNE if no agent $i \in \mc N^+$ (prosumers and DNO)  can reduce its cost  function $J_{i}(
     u_{i}^*, u_{-i}^*)$ by unilaterally changing its strategy	 $u_i^*$ to another feasible one, i.e., $u_i \in \mc U_i $ s.t. $(u_i,u_{-i}^*) \in \mc C$.
%\smallskip
%Since the cost functions \eqref{eq:cost_def} are continuous and the constraints $\mc U_i$'s are convex and compact, the existence of a GNE follows by Brouwer's fixed-point theorem \cite[Prop.~12.7]{palomar2010convex}, while uniqueness does not hold in general.

Note that, the cost functions in \eqref{eq:cost_def} are not influenced by some specific prosumers, but only by the local decisions and by the aggregative quantity $\sigma^{\textrm{mg}}$ in  \eqref{eq:Aggr}, namely, the active load on the main grid. Therefore, for each agent $i \in \mc I$, we can define a function $\tilde J_i$ such that
\begin{equation}
\tilde J_i(u_i, \sigma^{\textrm{mg}}) =:J_i(u_i,  u_{-i}).
\end{equation}
Games with such special structure are known as \textit{aggregative games} \cite{jensen2010aggregative,belgioioso2017convexity}, and have received intense research interest, within the operations research and the automatic control communities \cite{paccagnan2019,belgioioso2020semi,belgioioso2020b,
...
gadjov2020single,bianchi2020fast}. Among these methods, semi-decentralized ones \cite{belgioioso2020semi} have been shown to be particularly efficient in terms of convergence speed. % and general in terms of convergence guarantees.
...
%, while the step-size choices therein are formalized next.
%\smallskip
%\begin{assumption}\label{ass:SSS}
%The step sizes in Alg.~1 are set as follows:
%\begin{enumerate}[(i)]
%\item $\forall i \!\in \!\mc N$: $\alpha_i \!<\! 
%3 +N \underset{h\in \mc H}{\max} d_h^{\text{mg}} $, $\beta^{\text{tr}}_{(i,j)}\! =\! \beta^{\text{tr}}_{ (j,i)} \!<\! \frac{1}{2}$, $\forall j \!\in\! \mc N_i$. 
%
%\item $\alpha_{N+1} <2$, $\gamma^{\text{mg}} < \frac{1}{N}$, $\beta^{\text{tg}} < (|\mc N| + |\mc B|)^{-1}$ and $\beta_y^{\text{pb}} < (1+2|\mc N_y^{\mathrm{b}}|+|\mc B_y|)^{-1}$, for all $y \in \mc B$.
%{\hfill $\square$}
%\end{enumerate}
%\end{assumption}
%\smallskip
The next proposition shows its global convergence to a variational\footnote{Variational GNEs (v-GNEs) are  a special subclass of GNEs that enjoys the property of ``economic fairness”, namely, the marginal loss due to the presence of the coupling constraints is the same for each
agent, and coincides with the solutions to a specific variational inequality \cite{kulkarni2012variational}.} GNE of the P2P market game. %The algorithm derivation and its convergence analysis are discussed in Appendix \ref{app:CA}.
...
\text{For all busses } y \in \mc B \text{ (in parallel):}\\
 \left|
\begin{array}{l}
\mu_y^{\text{pb}}(k+1) \rightarrow \text{ all prosumers } i \in \mc N_y^{\text{b}}%
\end{array}%
\right.%
\end{array}%
\right.\\[1em]
...
\hrule
\smallskip
\textsc{Algorithm 2}. Prosumer $i$ local update
\smallskip
\hrule 
\smallskip
...
\text{Step sizes}: set $
 \alpha_i \!<\! 
({3} +N \max_{h}\, d_h^{\text{mg}})^{-1}$, $\beta^{\text{tr}}_{(i,j)}\! =\! \beta^{\text{tr}}_{ (j,i)} \!<\! \frac{1}{2}$.

\smallskip
\noindent 
(i) \text{Dual update:}\\[.2em]
\hspace*{.15em}
$
\left| 
\begin{array}{l}
...
\psi_i(k) = 
	\alpha_i \col \Big(-\mu_y^{\text{pb}}(k),\edit{\mu_y^{\text{pb}}(k), -\mu_y^{\text{pb}}(k),}\\
	\hspace*{4em}
	{
	\left[
	\begin{smallmatrix}
	...
	%
	\left\{ {\mu^{\text{tr}}_{(i,j)}(k)}\right\}_{j \in \mc N_i}   \Big)\\[1em]
%
\text{Set } u_i(k\!+\!1) \text{ as the unique solution to}\\[.3em]
\left\{
\begin{array}{r l}
	\underset{\xi \in \R^{n_i}}{\argmin} & 
	\tilde J_{i} \big( \xi, \sigma^{\text{mg}}(k) \big) 
	+ \frac{1}{2 \alpha_i}
	\left\| \xi - u_i(k) +  \psi_i(k) 
	\textstyle
	  \right\|^2 \\
...
\hspace*{8em} - p^{\text{tg}}_y(k\!+\!1) - \sum_{z \in \mc B_y} p^\ell_{(y,z)} (k\!+\!1)\\
%
...
%Such communication architecture is particularly efficient to design fast and scalable equilibrium seeking algorithms in games \cite{belgioioso2020semi};
