...
\ans{In this manuscript, we want to consider the most general problem formulation of peer-to-peer markets available in the literature. Following \cite{sorin2019,lecadre2020,baroche2019b}, a linear trading cost function is introduced in the problem formulation for the purpose of product differentiation or preference prices. %Furthermore, \cite{baroche2019} also adds a trading tariff or transaction cost encoded as a 1-norm cost function. 
 ...
 \ans{
\green{The average computational time for the parallel updates of each prosumer (i.e., Algorithm 2) is $t_{\text{avg}}^{\text{pro}} = \red{X}$; while for the DNO update (i.e., Algorithm 3), the average computational time is $t_{\text{avg}}^{\text{DNO}} = \red{X}$. Hence the total running time of the proposed market-clearing algorithm (Algorithm 1) to converge to a v-GNE configuration (i.e., reach a precision of \red{X}), when implemented on a real distributed system can be approximatively calculated as
\begin{align*}
...
\textit{\edit{`` The average computational time performed by each agent is 0.33 seconds per iteration on a computer with Intel Xeon E5-2637 3.5 GHz processors and 128 GB of memory.
		''}}
