...
\ans{We do not include energy loss of the distribution network in the current setup and leave it for future work. In this manuscript, we focus on the game-theoretic formulation of the interaction among prosumers and the network operator.}
...
\ans{\comment{Wicak}\comment{provide some references that consider this. Check dual variable as marginal price.}}
...
\ans{\comment{Wicak} \comment{Other possible approximations: linear distflow model or SDP. Future work?}}
...
\ans{\comment{Wicak} \comment{explain. Redo with a more interesting setup?}}
...
\ans{\comment{Wicak} \comment{show computational time}}
