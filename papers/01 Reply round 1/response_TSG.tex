...
\ans{We agree with the Reviewer. This work particularly shows how a distributed clearing mechanism can be implemented in such scenarios as well as how peer-to-peer market scenarios in theory can be beneficial.}
...
\ans{In view of the Reviewer's comment, we have updated the storage dynamics and included the charging and discharging efficiencies. Therefore, for each storage unit, we consider two decision variables, i.e., the charging and discharging powers. The storage unit is now modeled as follows:}

{``\edit{The battery charging and discharging profiles, $p_{i}^{\mathrm{ch}} = \col((p_{i,h}^{\mathrm{ch}})_{h\in \mc H})$ and $p_{i}^{\mathrm{ds}} = \col((p_{i,h}^{\mathrm{ds}})_{h\in \mc H})$, respectively, are constrained by the battery dynamics \cite{atzeni2013,zhong2019online}}}
%
\begin{equation}
%	\marginnote{\note{R1-3}\\ \note{R2-2}}
	\begin{aligned}
...
\ans{
%\green{
In the revised manuscript, (i) we have provided a detailed description of each individual variable along with an explanation of its function in the algorithm; (ii) we have re-designed the main algorithm layout; and (iii) we have included a paragraph to present the targeted class of equilibria considered. Please, refer to the margin notes (R1-5) in Section III for the related changes and additions.
%}
}
...
\ans{In this manuscript, we have considered the most general problem formulation of peer-to-peer markets available in the literature. Following \cite{sorin2019,lecadre2020,baroche2019b}, a linear trading cost function is introduced in the problem formulation for the purpose of product differentiation or preference prices. %Furthermore, \cite{baroche2019} also adds a trading tariff or transaction cost encoded as a 1-norm cost function. 
 ...
 \ans{
%\green{
	The average computational time (obtained using a PC with Intel Xeon E5-2637 3.5 GHz processors and 128 GB of memory) for the parallel updates of each prosumer (i.e., Algorithm 2) is $t_{\text{avg}}^{\text{pro}} = 74.5$ milliseconds; while for the DNO update (i.e., Algorithm 3), the average computational time is $t_{\text{avg}}^{\text{DNO}} = 1.13$  seconds. 
%
...
(ii) designing a more efficient ad-hoc algorithm to solve the projection step in Algorithm 3 (i.e., Algorithm 3 (i)); (iii) using a more computationally-powerful hardware to perform the DNO central updates, namely, Algorithm 3. Finally, we note that the main algorithm is scalable with respect to the number of prosumers, namely, the number of iterations to converge is not affected by the total number of prosumers in the network (see Figure {10}). Moreover, the computation time of the local updates is virtually also independent from the number of prosumers, for constant connectivity levels of the trading network. Therefore, the previous approximation remains valid independently of the market size.
%} 
}
...
\ans{In view of the Reviewer's comment, we have updated the storage dynamics, which include the charging and discharging efficiencies. Therefore, for each storage unit, we consider two decision variables, i.e., the charging and discharging powers. The storage unit is now modeled as follows:}
...
\ans{We agree with the Reviewer that the electricity price in (9) is more suitable for local energy systems than distribution networks.   Here, we consider the simplest model of electricity prices, which follows linear inverse demand function, i.e., the price increases when the demand does so. In other words, the cost function in (11) takes into account how congested the whole prosumer network is.   We note that the cost function defined by (9)--(11) already complicates the day-ahead problem compared to the existing P2P energy market literature \cite{lecadre2020,moret2020,sousa2019,baroche2019,cui2020,zhang2020,sorin2019}. To our knowledge, most of the existing literature on P2P energy markets exclude the possible interaction between the prosumer network and the main grid, and do not have coupled cost functions as in (11). In the revised manuscript, we acknowledge the inclusion of power line congestion and power loss in the formulation of electricity prices of the main grid as a potential future work. In addition, we may also generalize the grid cost function (9)--(11) by considering the total congestion per bus, i.e., the electricity price at each bus depends on the total demand of the prosumers connected to that particular bus.}
...
\ans{{We thank the Reviewer for drawing our attention on this relevant paper that we cited and briefly reviewed in the new version of the introduction.}}

{\edit{``A similar approach is considered in \cite{zhong2020cooperative}, which employs generalized Nash bargaining theory and decomposes  a P2P market problem into two hierarchical subproblems (a social welfare maximization and an energy trading problem).''
}}
...
\ans{{We have included a more detailed explanation of the targeted class of variational generalized Nash equilibria, including a formal mathematical definition, as follows.}}
...
\ans{ We agree with the Reviewer. Thus, following their suggestion, we have split the main algorithm in 3 parts: (1) (the revised) Algorithm 1, i.e., the main semi-decentralized loop, in which we summarize the information flow between prosumers and DNO, including the precise order of their updates; (2) Algorithm 2, i.e., the local primal-dual updates of the prosumers; (3) Algorithm 3, i.e., the central primal-dual update of the DNO.
%
Moreover, we have included a new paragraph in Section III-B to list all primal, dual, and auxiliary variables of (the revised) Algorithm 1, along with an explanation of their scopes within the algorithm and their economical and system-theoretic interpretations.
%
Finally, we have also re-designed the flow chart illustration of Algorithm 1 (Fig. 2).
%
Due to space limitations, we have omitted a more detailed discussion of the algorithm derivation. We refer the interested reader to \cite{belgioioso2020semi} for a complete discussion on this matter, including the analysis of the feasible step-sizes.
}
...
% highlighted the research gaps of the literature and rephrased 
improved the paragraphs on contribution to promote clarity.
The main advantage of game-theoretic energy market models with respect to optimization-based ones is their ability to formulate strategic (or self-interested) objectives, that naturally arise since prosumers are first and foremost self-interested decision-makers. Among game-theoretic models, GNE-based ones can additionally encode the global limitations of the grid (e.g., power-flow constraints, etc.) within the market.
%
...
%
Unfortunately, the design of market-clearing mechanisms for game-theoretic GNE-based market models was (until very recently) elusive, due to the huge computation and communication overhead that, in practice, makes these methods inadequate for implementation on real large-scale systems.
%
In this paper, we exploit some recent advances in algorithmic game theory for generalized games \cite{belgioioso2020semi} to design a decentralized and scalable market-clearing algorithm, and show, via numerical simulations, that it is suitable for implementation in real large-scale systems.
}
}
...
\ans{As discussed in \cite[Sect. 3.2]{sousa2019}, in community-based local markets, each prosumer  does not know with which other prosumers it trades energy, and the power balance of the whole network (all prosumers) is considered. Furthermore, a coordinator is responsible for managing the trading activities in the community. On the other hand, our setup considers full P2P markets where each prosumer knows its trading partners and, thus, negotiates directly with them. The coordinator in our setup plays a role in handling the physical constraints of the network as well as the aggregated power bought from the main grid. We have added this discussion as Remark 3 in the revised manuscript.}
...
\ans{For the simulations in Section IV-C, we consider a uniform per-unit trading price $c_{(i,j)}^{\mathrm{tr}}=0.08$, for all trading links $(i,j) \in \mc E$, and trading tariff $c^{\mathrm{ta}}=0.01$. Figure 9 in the revised manuscript show how the average trading prices $c^{\mathrm{tr}} + c^{\mathrm{ta}} + \tfrac{1}{|\mc E|}\sum_{(i,j)\in \mc E}\mu_{(i,j)}^{\mathrm{tr}}$ compare with the electricity price from the main grid ($c^{\mathrm{mg}}$). The relatively low trading prices explain high power traded among the prosumers. We can also observe from the top plot that during the first six hours, these prices are almost equal, thus the amount of power traded is relatively low (see Figure 8 in the revised manuscript). }
...
\ans{We thank the Reviewer for providing this observation. What we claim is that the storage shaves the peak of the sum of the total power imported from the main grid and that of locally generated power. Due to the choice of the cost parameters, which follows \cite{atzeni2013}, local generation costs are cheaper than importing cost. We have redone the simulations where we readjust the cost parameters and introduced a positive quadratic coefficient on $f_i^{\mathrm{di}}$. Based on this adjustment, we can now see in Figure 7 in the revised manuscript (as also shown below) that the storage units particularly shave the peak of total power imported from the main grid.}
...
\ans{We thank the Reviewer for the very positive comment and recommendation.}
...
\ans{We agree with the Reviewer that the statement should be made clearer. That statement concerns the analysis for the algorithm development as we now have a larger set of coupling constraints than that in \cite{belgioioso2020energy}. In order to have an efficient algorithm, we exploit the structure of these constraints in decomposing the problem, thus resulting in Algorithm 1 in the revised manuscript, which is more complicated than \cite[Algorithm 1]{belgioioso2020energy}. Based on this discussion, in the revised manuscript, we have modified the statement as follows:}

{ \edit{``Our market formulation extends our preliminary work \cite{belgioioso2020energy} by including network operational constraints and system operators in the model, which complicate the analysis as we need to exploit the problem structure to derive an efficient algorithm.''}}
...
\ans{We thank the Reviewer for raising this point. For our proposed algorithm, we have a convergence guarantee for either linear or non-linear objective functions as long as they are locally convex, i.e., convex with respect to local decision variables. In other words, our proposed algorithm works with quadratic local cost functions. Nevertheless, in view of this comment, we have updated the simulations in Section IV-C, where we now use a quadratic cost of dispatchable production, i.e., $Q_{i}^{\mathrm{di}}>0$, for all $i \in \mc N^{\mathrm{di}}$, which varies from one unit to another. The results are shown in Figure {7}, where we still observe a peak-shaving effect caused by the introduction of storage units. As this manuscript focuses on the distributed clearing mechanism design, we leave the analysis of the effect of different possible objective functions on the clearing outcome as future work.}
...
\ans{We thank the Reviewer for raising this point. We agree that the computational time is relevant in practice and thus we have added the remark below in Section IV-C. Please, refer to our answer to the comment [R1-7] for a detailed discussion on the applicability of the proposed market-clearing mechanism on real large-scale systems.} %\red{particularly for the simulations with different numbers of agents} 

{\edit{`` The average computation times of the inner loops, i.e., Algorithm~2 and 3, obtained on a computer with Intel Xeon E5-2637 3.5 GHz processors and 128 GB of memory, are 74.5 ms and 1.13 s, respectively.
		''}}
%
 ...
 \query[R4-8]{ Given a prosumer $i$, when Algorithm 1 is applied, in order to update the dual variable related to reciprocity constraints, the value of the power traded with neighbors plays an important role. In detail, I mean $\{p^{tr}_{(j,i)}\}_{j\in \mc N_{i}}$, the forward trade estimate to prosumer $i$. It seems that, depending on the order the prosumers are considered, the coordination of these variables could change, so that the convergence of the algorithm. Hence, it could be interesting to analyze how this affect to the behavior of the algorithm. For example, could it benefit to consider first prosumers with less neighbors, since maybe they reach faster an equilibrium? }
 	\ans{{
In the proposed market-clearing mechanism, all the local (primal and dual) updates of the prosumers are performed in parallel. We explicitly clarified this point in the new algorithm layout (please see the revised Algorithm 1).
 	}}
 ...
 \textit{Minor comments:}
 	...
 	\edit{{``For any $x \in \bb R^n$, $\|x\|_A^2 = x^\top A x,$ with square symmetric matrix $A \succ 0$.''}}
 	...
 	\ans{We thank the Reviewer for noticing this errors. As we have updated the presentation of Algorithm 1 to improve readability, we have also modified Remark 2 accordingly.}
