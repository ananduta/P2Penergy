function [s,sl,np] = sd_alg_eP2P1(np)
...
% 03/12/2020
        ...
        for i=1:np.n
            % decision variables
            s = initialize_u_quadp1(np,s,i);

            sl.b{i}(:,1) = np.Pd(i,1:np.h)' - s.p_di{i}(:,1) - s.p_st{i}(:,1);

            s.sigma_mg(:,1) = s.sigma_mg(:,1) + s.p_mg{i}(:,1) + s.p_mgs{i}(:,1);
        end
        sl.lambda_mg= zeros(2*np.h,1);
        ...
        while 1
           ...
           for i=1:np.n
            ...
            for i=1:np.n
                ...
                for j=1:np.n
                    if np.Adj(i,j) == 1
                        ...
                        dres2(t) = dres2(t) + (sl.mu_tr{i,j}(:,t+1) - sl.mu_tr{i,j}(:,t))'*(sl.mu_tr{i,j}(:,t+1) - sl.mu_tr{i,j}(:,t)); 
                    end
                end
            end
            ...
            sl.lambda_mg(:,t+1) = max(0, sl.lambda_mg(:,t) + np.gamma_mg*sl.b_DSO(:,t+1));        
