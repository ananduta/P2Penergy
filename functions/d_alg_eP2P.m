...
% 17/09/2020
        ...
        for y = 1:np.b
            ...
            s.c_pb{y}(:,1) = np.Pd(np.n+y,:)' - s.p_tg{y}(:,1);
            ...
            s.sum_p_pd = s.sum_p_pd + np.Pd(np.n+y,:)';

        end
        s.sigma_mg(:,1) = s.sigma_mg(:,1);% + s.sum_p_pd;

        s.c_tg(:,1) = zeros(np.h,1);
        ...
        while 1
           ...
           for i=1:np.n
            ...
            for y=1:np.b
                s.a_DSO{y}(:,t) = [zeros(2*np.h,1); -s.mu_tg(:,t);kron(ones(length(np.B{y}),1),s.mu_pb{y}(:,t));zeros(np.h*length(np.B{y}),1)]; 
            end
...
%            s = projDSO_comp(s,np,t);                                      
            ...
            for y=1:np.b
                s.c_pb{y}(:,t+1) = np.Pd(np.n+y,:)' - s.p_tg{y}(:,t+1);
                for ii = 1:length(np.N_b{y})
                    i = np.N_b{y}(ii);
