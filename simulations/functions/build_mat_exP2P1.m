function np = build_mat_exP2P1(np)
% Building matrices for local optimization 
% for prosumers of extended P2P (in the cost and constraints)
% W. Ananduta
% 04/12/2020

[np.Sdi,np.Sst,np.Smg,np.Smgs,np.Str] = gen_Smat1(np);

for i=1:np.n
    Ni = sum(np.Adj(i,:));

    % Cost function
    Q = diag([np.q_dg(i) np.q_st(i) 0 0 zeros(1,Ni)]);
    ...
    cc = 1;
    c = [np.c_dg(i) np.c_st(i) 0 0 zeros(1,Ni)]';
    for j=1:np.n 
        if np.Adj(i,j) == 1
            c(4+cc,1) = np.c_tr(i,j);
            cc = cc+1;
        end
    end
    ...
    if np.st_un(i) == 1
        ...
        Am = np.a_st(i);
        Bm = [0 -1 0 0 zeros(1,Ni)]; %be careful when sampling time is not 1 hour
