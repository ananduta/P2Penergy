function s = loc_opt_qprog_l1_fminc(np,s,sl,t,i)
% Local optimization (using fmincon)
% W. Ananduta
% 02/12/2019
...
%for i = 1:np.n
    ...
    for h = 1:np.h
        ...
        c_1 = c + [0; 0; np.d_mg*(s.sigma_mg(h,t)); zeros(sum(np.Adj(i,:)),1)]; 
        c1 = [c1;c_1];
    end
    ...
    c5 = zeros(np.h*(3+sum(np.Adj(i,:))),1);
    for j=1:np.n
        if np.Adj(i,j) == 1
...
f = np.c{i};
Ni = sum(np.Adj(i,:));
Str = [zeros(Ni,3) eye(Ni)];
Str = kron(eye(np.h),Str);
Jc = @(x) 0.5*x'*H*x + f'*x + np.q_tr*norm(Str*x,1);
x0 = zeros(np.h*(3+Ni),1);
options = optimset('Display','off');
%options = optimset('Display','on');
[u_i,fval,exitflag] = fmincon(Jc,x0,A,b,Aeq,beq,[],[],[],options);
