function s = loc_opt_qprog_D(np,s,sl,t,i)
...
%for i = 1:np.n
    ...
    for h = 1:np.h
        ...
        c_1 = c + [0; 0; np.d_mg*(s.sum_sigma(h,t)); zeros(sum(np.Adj(i,:)),1)];
        c1 = [c1;c_1];
    end
    ...
    c4 = sl.lambda_mg{np.B_n(i)}(:,t)'*[np.Smg{i};-np.Smg{i}]+sl.mu_tg{np.B_n(i)}(:,t)'*np.Smg{i};
    c4 = c4';
